...
-- Backward-compatible migration for existing databases where the profile table
-- already exists but does not yet have the soft-delete columns.
...
declare
  signup_role text;
begin
  signup_role := lower(coalesce(new.raw_user_meta_data->>'signup_role', 'buyer'));
  ...
  on conflict (id) do update
    set email = excluded.email,
        ...
        settings = coalesce(public.profiles.settings, '{}'::jsonb),
        updated_at = now();
...
create policy "profiles_select_own" on public.profiles
  for select using (auth.uid() = id and coalesce(is_deleted, false) = false);
...
create policy "profiles_update_own" on public.profiles
  for update using (auth.uid() = id and coalesce(is_deleted, false) = false)
  with check (
    auth.uid() = id
