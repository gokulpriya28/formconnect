...
import { LanguageProvider, useLanguage, LanguageToggle } from "./src/i18n.jsx";
...
function ConsentBanner({ onAccept, onViewPolicy, onViewTerms }) {
  ...
  return (
    <div className="consent-banner" role="dialog" aria-label="Privacy consent">
      <p>
        {t("consent_text")}{" "}
        <a onClick={onViewPolicy}>{t("privacy_policy")}</a> &middot;{" "}
        <a onClick={onViewTerms}>{t("terms_of_service")}</a>
      </p>
    ...
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div className="modal-title">{t("privacy_policy")}</div>
          <button className="btn btn-ghost btn-sm" onClick={onClose}>✕</button>
        </div>
        {lang === "ta" && (
          <div style={{fontSize:12,color:G.stone,marginBottom:12,padding:"8px 12px",background:G.mist,borderRadius:8}}>
            இந்தச் சட்ட ஆவணம் தற்போது ஆங்கிலத்தில் மட்டுமே கிடைக்கிறது. (This legal document is currently available in English only.)
          </div>
        )}
        ...
        <button className="btn btn-primary" style={{marginTop:20,width:"100%"}} onClick={onClose}>{t("i_understand")}</button>
      </div>
    </div>
  );
}
...
function TermsModal({ onClose }) {
  ...
  return (
    <div className="overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-lg">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div className="modal-title">{t("terms_of_service")}</div>
          <button className="btn btn-ghost btn-sm" onClick={onClose}>✕</button>
        </div>
        {lang === "ta" && (
          <div style={{fontSize:12,color:G.stone,marginBottom:12,padding:"8px 12px",background:G.mist,borderRadius:8}}>
        ...
        <button className="btn btn-primary" style={{marginTop:20,width:"100%"}} onClick={onClose}>{t("i_agree")}</button>
      </div>
    </div>
  );
}
...
// ─── COMPONENTS ──────────────────────────────────────────────────
const STATUS_KEY_MAP = { delivered:"status_delivered", "in-transit":"status_in_transit", confirmed:"status_confirmed", pending:"status_pending", cancelled:"status_cancelled" };

function StatusPill({ s }) {
  const { t } = useLanguage();
  const c = statusConfig[s] || { cls:"pill-gray", icon:"·", label:s };
  const label = STATUS_KEY_MAP[s] ? t(STATUS_KEY_MAP[s]) : c.label;
  return <span className={`pill ${c.cls}`}><span className="dot" />{label}</span>;
}
    ...
    <div className="overlay" onClick={e => e.target===e.currentTarget && onClose()}>
      <div className="modal">
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
          <div>
            <div className="modal-title">{t("place_order")}</div>
            <div className="modal-sub">{t("escrow_protected")}</div>
          </div>
        ...
        <div className="modal-produce-row">
          ...
          <div>
            ...
            <div style={{marginTop:4}}>
              {item.organic && <span className="badge organic">{t("organic")}</span>}
              {item.express && <span className="badge express" style={{marginLeft:4}}>{t("express")}</span>}
            </div>
          </div>
          <div style={{marginLeft:"auto",textAlign:"right"}}>
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:22,color:G.amber,fontWeight:500}}>₹{item.price}</div>
            <div style={{fontSize:11,color:G.stone}}>{t("per")} {item.unit}</div>
            <div style={{fontSize:11,color:"#DC2626",textDecoration:"line-through"}}>{t("mandi")}: ₹{item.msP}</div>
          </div>
        </div>
        <div className="form-group">
          <label className="form-label">{t("quantity")} ({item.unit})</label>
          ...
          <div className="form-hint">Min 10 {item.unit} · {t("available")}: {item.qty} {item.unit}</div>
        </div>
        <div className="order-summary">
          <div className="summary-row"><span>{t("subtotal")} ({qty} {item.unit} × ₹{item.price})</span><span>₹{subtotal.toLocaleString()}</span></div>
          <div className="summary-row highlight"><span>{t("farmer_earns")}</span><span>₹{farmer.toLocaleString()}</span></div>
          <div className="summary-row highlight"><span>{t("platform_fee_line")}</span><span>₹{platform.toLocaleString()}</span></div>
          <div className="summary-row tax"><span>{t("gst_18")}</span><span>₹{gst.toLocaleString()}</span></div>
          <div className="summary-row total"><span>{t("total_payable")}</span><span style={{color:G.amber}}>₹{total.toLocaleString()}</span></div>
        </div>
        <div className="alert alert-green">{t("payment_held_escrow")}</div>
        <div style={{display:"flex",gap:10}}>
          ...
          <button className="btn btn-amber" onClick={() => !qtyError && onOrder(item, qty, total)} disabled={!!qtyError} style={{flex:2}}>
            {t("pay_via_upi", { total: total.toLocaleString() })}
          </button>
        </div>
      </div>
    </div>
...
function FarmerDashboard() {
  ...
  const nav = [
    ...
    { key:"listings", icon:"📦", label:t("nav_my_listings") },
    { key:"add", icon:"➕", label:t("nav_add_produce") },
    { key:"orders", icon:"📋", label:t("nav_orders") },
    { key:"earnings", icon:"💰", label:t("nav_earnings") },
    { key:"schemes", icon:"🏛️", label:t("nav_govt_schemes") },
  ];
  ...
  return (
    <div className="main">
      ...
      <div className="content">
        {view==="home" && <>
          <div className="page-title">{t("good_morning")}</div>
          <div className="page-subtitle">{t("farm_summary")} · June 20, 2024</div>
          <div className="wallet-card">
            <div className="wallet-label">{t("this_month_earnings")}</div>
          ...
          <div className="grid-4">
            <div className="card"><div className="card-title">{t("active_listings")}</div><div className="card-value">3</div><div className="card-delta">↑ 2 new this week</div></div>
            <div className="card"><div className="card-title">{t("pending_orders")}</div><div className="card-value amber">2</div><div className="card-delta">Needs action</div></div>
            <div className="card"><div className="card-title">{t("rating")}</div><div className="card-value">4.8★</div><div className="card-delta">32 reviews</div></div>
            <div className="card"><div className="card-title">{t("nav_govt_schemes")}</div><div className="card-value">2</div><div className="card-delta">PM-KISAN eligible</div></div>
          </div>
          ...
          <div className="table-wrap">
            <div className="table-header"><div className="table-title">{t("recent_orders")}</div></div>
            <table>
              <thead><tr><th>{t("order_id")}</th><th>{t("buyer")}</th><th>{t("produce")}</th><th>{t("amount")}</th><th>{t("status")}</th></tr></thead>
              <tbody>
                {myOrders.map(o => (
                  <tr key={o.id}>
        ...
        {view==="add" && <>
          <div className="page-title">{t("nav_add_produce")}</div>
          ...
          <div style={{display:"flex",gap:12}}>
            <button className="btn btn-outline">{t("save_as_draft")}</button>
            <button className="btn btn-primary" disabled={uploadProgress} onClick={handlePublishListing}>
              {uploadProgress ? t("uploading") : t("publish_listing")}
            </button>
          </div>
        </>}

        {view==="listings" && <>
          <div className="page-title">{t("my_listings")}</div>
              ...
              <div className="produce-card" key={p.id}>
                ...
                <div className="produce-body">
                  ...
                  <div className="produce-price-row">
                    ...
                    <button className="btn btn-ghost btn-sm" style={{fontSize:11}}>{t("edit")}</button>
                  </div>
                  ...
                  <div style={{marginTop:8,display:"flex",justifyContent:"space-between",fontSize:11,color:G.stone}}>
                    <span>{t("qa_score")}</span><span style={{fontWeight:700,color:G.leaf}}>{p.qaScore}/100</span>
                  </div>
                  <div style={{marginTop:4,display:"flex",justifyContent:"space-between",fontSize:11,color:G.stone}}>
                    <span>{t("grade")}</span><span style={{fontWeight:700,color:G.amber}}>{t("grade")} {p.qaGrade}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        {view==="orders" && <>
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>{t("order_id")}</th><th>{t("buyer")}</th><th>{t("produce")}</th><th>{t("quantity")}</th><th>{t("amount")}</th><th>{t("your_share")}</th><th>{t("date")}</th><th>{t("status")}</th></tr></thead>
              <tbody>
                {myOrders.map(o => (
                  <tr key={o.id}>
        ...
        {view==="earnings" && <>
          <div className="page-title">{t("earnings_payments")}</div>
        ...
        {view==="schemes" && <>
          <div className="page-title">{t("govt_schemes")}</div>
...
function BuyerDashboard() {
  ...
  const nav = [
    { key:"browse", icon:"🛒", label:t("nav_browse_market") },
    { key:"orders", icon:"📋", label:t("nav_my_orders") },
    { key:"track", icon:"🚛", label:t("nav_track_delivery") },
    { key:"invoices", icon:"🧾", label:t("nav_invoices") },
  ];
  ...
  return (
    <div className="main">
      ...
      <div className="content" style={{background:G.sky}}>
        {view==="browse" && <>
          <div className="page-title">{t("fresh_market")}</div>
          <div className="page-subtitle">{t("fresh_market_sub")}</div>
          <div className="search-bar">
            <span className="search-icon">🔍</span>
            <input className="search-input" placeholder={t("search_placeholder")}
          ...
          <div className="filter-row">
            ...
            <button className={`filter-chip${filter==="express"?" active":""}`} onClick={() => setFilter("express")} style={filter==="express"?{background:"#6D28D9",borderColor:"#6D28D9",color:"white"}:{}}>{t("express_delivery")}</button>
            <button className={`filter-chip${filter==="organic"?" active":""}`} onClick={() => setFilter("organic")} style={filter==="organic"?{background:"#92400E",borderColor:"#92400E",color:"white"}:{}}>{t("organic_only")}</button>
          </div>
          <div className="produce-grid">
            {filtered.map(p => (
              <div className="produce-card" key={p.id} onClick={() => setModal(p)}>
                ...
                <div className="produce-body">
                  ...
                  <div className="produce-badges">
                    {p.organic && <span className="badge organic">🌿 {t("organic")}</span>}
                    {p.express && <span className="badge express">⚡ {t("express")}</span>}
                    <span className="badge verified">✓ Verified</span>
                  ...
                  <div style={{marginTop:10}}>
                    <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:G.stone,marginBottom:3}}>
                      <span>{t("stock")}</span><span>{p.qty} {p.unit}</span>
                    </div>
        ...
        {view==="orders" && <>
          ...
          <div className="table-wrap" style={{background:"white"}}>
            <table>
              <thead><tr><th>{t("order_id")}</th><th>{t("produce")}</th><th>{t("farmer")}</th><th>{t("quantity")}</th><th>{t("amount")}</th><th>{t("gst_govt")}</th><th>{t("date")}</th><th>{t("status")}</th><th>{t("action")}</th></tr></thead>
              <tbody>
                {orders.map(o => (
                  <tr key={o.id}>
        ...
        {view==="track" && <>
          <div className="page-title">{t("track_delivery")}</div>
        ...
        {view==="invoices" && <>
          <div className="page-title">{t("gst_invoices")}</div>
            ...
            <div className="form-section" key={o.id} style={{marginBottom:12}}>
              <div style={{display:"flex",justifyContent:"space-between"}}>
                ...
                <div style={{textAlign:"right"}}>
                  ...
                  <button className="btn btn-primary btn-sm" style={{marginTop:8}} onClick={() => showToast("Invoice PDF downloaded!")}>{t("download_pdf")}</button>
                </div>
              </div>
            </div>
          ))}
...
function AdminDashboard({ role }) {
  ...
  const nav = isGovt ? [
    { key:"overview", icon:"🏛️", label:t("nav_overview") },
    { key:"revenue", icon:"💰", label:t("nav_tax_revenue") },
    { key:"farmers", icon:"🌾", label:t("nav_farmer_data") },
    { key:"schemes", icon:"📋", label:t("nav_scheme_impact") },
  ] : [
    { key:"overview", icon:"📊", label:t("nav_overview") },
    { key:"transactions", icon:"💳", label:t("nav_all_transactions") },
    { key:"farmers", icon:"🌾", label:t("nav_farmers") },
    { key:"users", icon:"👤", label:t("nav_users") },
    { key:"audit", icon:"🔍", label:t("nav_audit_logs") },
    { key:"disputes", icon:"⚠️", label:t("nav_disputes") },
  ...
  return (
    <div className="main">
      ...
      <div className="sidebar" style={{background: isGovt ? "#1A237E" : G.soil}}>
        ...
        <div className="sidebar-section" style={{color:"rgba(255,255,255,0.4)"}}>{t("menu")}</div>
        {nav.map(n => (
      ...
      <div className="content">
        {view==="overview" && <>
          {isGovt ? (
            <div className="govt-header">
              <div className="govt-title">{t("govt_revenue_dashboard")}</div>
            ...
            <><div className="page-title">{t("platform_overview")}</div><div className="page-subtitle">FarmConnect Admin · June 20, 2024</div></>
          )}
          <div className="grid-4">
            <div className="card"><div className="card-title">{t("total_gmv")}</div><div className="card-value">₹{totalGMV.toLocaleString()}</div><div className="card-delta">↑ 42% MoM</div></div>
            <div className="card"><div className="card-title">{t("platform_revenue")}</div><div className="card-value amber">₹{Math.round(totalGMV*0.05).toLocaleString()}</div><div className="card-delta">↑ 38% MoM</div></div>
            <div className="card"><div className="card-title">{t("govt_gst_revenue")}</div><div className="card-value" style={{color:"#1D4ED8"}}>₹{totalGST.toLocaleString()}</div><div className="card-delta">Transparent · Auto-filed</div></div>
            <div className="card"><div className="card-title">{t("active_farmers")}</div><div className="card-value">2,847</div><div className="card-delta">↑ 312 this month</div></div>
          </div>
          <div className="flow">
        ...
        {view==="transactions" && <>
          <div className="page-title">{t("all_transactions")}</div>
          <div className="page-subtitle">Complete audit trail · Every rupee tracked</div>
          <div className="table-wrap">
            <table>
              <thead><tr><th>Txn ID</th><th>{t("buyer")}</th><th>{t("farmer")}</th><th>{t("produce")}</th><th>{t("amount")}</th><th>{t("farmer")} (90%)</th><th>Platform (5%)</th><th>GST</th><th>{t("date")}</th><th>{t("status")}</th></tr></thead>
              <tbody>
                {orders.map(o => (
                  <tr key={o.id}>
        ...
        {!isGovt && view==="users" && <>
          <div className="page-title">{t("user_access_control")}</div>
        ...
        {!isGovt && view==="audit" && <>
          <div className="page-title">{t("audit_logs")}</div>
        ...
        {view==="farmers" && <>
          <div className="page-title">{t("farmer_directory")}</div>
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>{t("farmer")}</th><th>Village</th><th>Crops</th><th>Monthly Income</th><th>Income Boost</th><th>{t("rating")}</th><th>{t("status")}</th></tr></thead>
              <tbody>
                {produceItems.map(p => (
                  <tr key={p.id}>
        ...
        {view==="revenue" && isGovt && <>
          <div className="page-title">{t("tax_revenue_breakdown")}</div>
        ...
        {view==="schemes" && isGovt && <>
          <div className="page-title">{t("scheme_impact_report")}</div>
        ...
        {view==="compliance" && <>
          <div className="page-title">{t("compliance_center")}</div>
        ...
        {view==="disputes" && <>
          <div className="page-title">{t("disputes")}</div>
...
function AppShell() {
      ...
      <div className="app">
        ...
        <div className="topbar">
          ...
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            <div style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}>{t("brand_tagline")}</div>
            <LanguageToggle />
          </div>
        </div>
        <div className="landing">
          <div className="landing-hero">
            <h1>{t("hero_title_1")}<br/>{t("hero_title_2")} <em>{t("hero_title_3")}</em> {t("hero_title_4")}</h1>
            <p>{t("hero_sub")}<br/>{t("hero_sub_2")}</p>
          </div>

          <div className="flow" style={{padding:"16px 24px",gap:12}}>
            <div className="flow-node"><div className="flow-node-title">{t("flow_farmer")}</div><div className="flow-node-sub">{t("flow_farmer_sub")}</div></div>
            <div className="flow-arrow">→</div>
            <div className="flow-node highlight"><div className="flow-node-title">{t("flow_platform")}</div><div className="flow-node-sub">{t("flow_platform_sub")}</div></div>
            <div className="flow-arrow">→</div>
            <div className="flow-node"><div className="flow-node-title">{t("flow_hotel")}</div><div className="flow-node-sub">{t("flow_hotel_sub")}</div></div>
            <div className="flow-arrow">→</div>
            <div className="flow-node amber"><div className="flow-node-title">{t("flow_govt")}</div><div className="flow-node-sub">{t("flow_govt_sub")}</div></div>
          </div>

          {/* ── Auth Form ── */}
          <div className="form-section" style={{maxWidth: 460, width: "100%"}}>
            <div className="form-section-title">{t("secure_auth")}</div>
            ...
            <div style={{display:"flex",gap:8,marginBottom:12}}>
              <button id="btn-signin" type="button" className={`btn ${authMode==="signin"?"btn-primary":"btn-outline"}`} onClick={() => { setAuthMode("signin"); setPwErrors([]); setAuthMessage(""); }}>{t("sign_in")}</button>
              <button id="btn-signup" type="button" className={`btn ${authMode==="signup"?"btn-primary":"btn-outline"}`} onClick={() => { setAuthMode("signup"); setPwErrors([]); setAuthMessage(""); }}>{t("sign_up")}</button>
            </div>
            ...
            <form onSubmit={handleAuthSubmit} autoComplete="on" noValidate>
              ...
              {authMode === "signup" && (
                <>
                  ...
                  <div style={{fontSize:12,color:G.stone,marginBottom:12,padding:"10px 12px",background:G.mist,borderRadius:8}}>
                    ...
                    <a style={{color:G.leaf,cursor:"pointer"}} onClick={() => setShowTerms(true)}>{t("terms_of_service")}</a> {t("and")}{" "}
                    <a style={{color:G.leaf,cursor:"pointer"}} onClick={() => setShowPolicy(true)}>{t("privacy_policy")}</a>.
                  </div>
                </>
              )}
              ...
              <button id="btn-auth-submit" className="btn btn-primary" type="submit"
                ...
                {authLoading ? t("working") : authMode === "signup" ? t("create_account") : t("sign_in")}
              </button>
            </form>
          ...
          <div className="role-cards">
            {getVisibleRoleCards(profileRole).map(({ role, icon, badgeStyle }) => (
              <div
                key={role}
                ...
                tabIndex={0}
                role="button"
                aria-label={`${t("enter_dashboard")} ${t(`role_${role}_label`)}`}
              >
                <div className="role-icon">{icon}</div>
                <div className="role-title">{t(`role_${role}_label`)}</div>
                <div className="role-desc">{t(`role_${role}_desc`)}</div>
                <div className="role-tag" style={badgeStyle}>{t(`role_${role}_badge`)}</div>
              </div>
            ))}
          </div>

          <div style={{display:"flex",gap:24,flexWrap:"wrap",justifyContent:"center",marginTop:8}}>
            {[["2,847",t("verified_farmers")],["₹42L",t("monthly_gmv")],["16.7%",t("farmer_income_boost")],["₹6,700",t("gst_to_govt")]].map(([v,l]) => (
        ...
        {showForgotPw && (
          <div className="overlay" onClick={e => e.target===e.currentTarget && setShowForgotPw(false)}>
            <div className="modal" style={{maxWidth:400}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                <div className="modal-title" style={{fontSize:20}}>{t("reset_password")}</div>
                <button className="btn btn-ghost btn-sm" onClick={() => setShowForgotPw(false)}>✕</button>
              </div>
              <p style={{fontSize:13,color:G.stone,marginBottom:16}}>{t("reset_password_desc")}</p>
              {forgotStatus && <div className={`alert ${forgotStatus.includes("sent") ? "alert-green" : "alert-amber"}`}>{forgotStatus}</div>}
              <div className="form-group">
                <label className="form-label" htmlFor="forgot-email">{t("email_address")}</label>
              ...
              <button className="btn btn-primary" style={{width:"100%"}} onClick={handleForgotPassword}>{t("send_reset_email")}</button>
            </div>
          </div>
        )}
        ...
        {showDeleteAccount && (
          <div className="overlay" onClick={e => e.target===e.currentTarget && setShowDeleteAccount(false)}>
            <div className="modal" style={{maxWidth:420}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                <div className="modal-title" style={{fontSize:20,color:G.red}}>{t("delete_account")}</div>
              ...
              <div style={{display:"flex",gap:10}}>
                <button className="btn btn-outline" style={{flex:1}} onClick={() => setShowDeleteAccount(false)}>{t("cancel")}</button>
                <button className="btn btn-danger" style={{flex:1}} onClick={handleDeleteAccount}>{t("yes_delete_account")}</button>
              </div>
            </div>
          </div>
        )}
  ...
  return (
    <div className="app">
      ...
      <div className="topbar">
        <div className="topbar-brand">🌱 Farm<span>Connect</span></div>
        <div className="topbar-nav" style={{alignItems:"center"}}>
          {getVisibleRoleCards(profileRole).map(({ role: roleKey, icon }) => (
            <button
              key={roleKey}
              className={`nav-btn${role===roleKey?" active":""}`}
              onClick={() => openRole(roleKey)}
            >
              {icon} {t(`role_${roleKey}_label`)}
            </button>
          ))}
          <button className="nav-btn" onClick={() => setRole(null)} style={{color:"rgba(255,255,255,0.4)"}}>{t("home")}</button>
          {session && user && <span className="nav-btn" style={{color:"rgba(255,255,255,0.85)",cursor:"default"}}>{user.email}</span>}
          {session && <button className="nav-btn" onClick={handleSignOut}>{t("sign_out")}</button>}
          <LanguageToggle style={{marginLeft:6}} />
        </div>
      </div>
